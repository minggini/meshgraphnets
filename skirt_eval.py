...
def _rollout(model, initial_state, num_steps):
    ...
    def step_fn(cur_pos, cur_vel, trajectory):
        # memory_prev = torch.cuda.memory_allocated(device) / (1024 * 1024)
        with torch.no_grad():
            prediction = model({**initial_state,
                                'cloth_pos': cur_pos,
                                'cloth_vel': cur_vel}, is_training=False)
        
        next_pos = torch.where(mask, torch.squeeze(prediction), torch.squeeze(prediction))
    ...
    #prev_pos = torch.squeeze(initial_state['prev|cloth_pos'], 0)
    cur_pos = torch.squeeze(initial_state['cloth_pos'], 0)
    cur_vel = torch.squeeze(initial_state['cloth_vel'], 0)
    trajectory = []
    for step in range(num_steps):
        prev_pos, cur_pos, trajectory = step_fn(cur_pos, cur_vel, trajectory)
